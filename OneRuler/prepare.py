...
parser.add_argument("--inst_lang", type=str, default='en', help='language')
...
def main():
    ...
    with open(os.path.join(curr_folder, f"../config/{args.benchmark}.yaml"), "r") as f:
    ...
    if not file_exists:
        try:
            ...
            --subset {args.subset} \
            --lang {args.lang}\
            --tokenizer_path {args.tokenizer_path.strip()} \
            ...
            {f"--xling" if args.xling else ""} \
            {f"--inst_lang {args.inst_lang}" if args.xling else ""} \
            """
            print(command)
            try:
